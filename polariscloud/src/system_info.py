    ...
    try:
        ...
        try:
            ...
            if lspci_vga.stdout.strip():
                ...
                cleaned_name = re.sub(r'\(rev [^\)]+\)', '', gpu_name).strip()
                cleaned_name = re.sub(r'\(prog-if [^\)]+\)', '', cleaned_name).strip()
                raw_logger.info(f"Cleaned GPU name: '{cleaned_name}'")
                ...
                raw_logger.info(f"Final simplified lspci GPU info: {result}")
        ...
        if is_windows():
            ...
            try:
                ...
                if 'Apple' in gpu_info:
                    # Apple Silicon has integrated GPU, but it's quite powerful
                    raw_logger.info("Found Apple Silicon GPU")
                    return True
